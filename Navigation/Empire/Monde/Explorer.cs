...
function explorerMonde() {
    try {
        ...
        if (!gameState.empire) {
            showNotification('Syst̬me d''empire non disponible', 'error');
            return false;
        }
        
        if (!gameState.territories) {
            showNotification('Donn̩es territoriales non disponibles', 'error');
            return false;
        }

        // Actions sp̩cifiques
        // V̩rifier les ressources n̩cessaires pour l'exploration
        const coutExploration = 100; // Or n̩cessaire
...
function creerInterfaceExplorerMonde() {
    const container = document.createElement('div');
    container.className = 'explorermonde-interface';
            ...
            <div class="action-content">
                ...
                <div class="action-details">
                    ...
                    <div class="info-section">
                        <p>Fonctionnalit̩: </p>
                        <p>Cat̩gorie: Empire</p>
                    </div>
                </div>
            </div>
            <div class="action-buttons">
                <button class="btn primary" onclick="explorerMonde()">
                    Ex̩cuter
                </button>
...
if (typeof window !== 'undefined') {
    ...
    window.creerInterfaceExplorerMonde = creerInterfaceExplorerMonde;
}

console.log('Module explorerMonde charg̩');
